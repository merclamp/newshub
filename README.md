...
Агрегатор новостей независимых СМИ: статьи с сайтов и новые видео с YouTube-каналов в одной ленте, плюс Telegram-бот с периодическими дайджестами.
...
| Источник | Статьи (RSS) | Видео (YouTube RSS) |
|---|---|---|
| DW Россия | ✓ | ✓ |
| Meduza | ✓ | ✓ (MeduzaLive) |
| Дождь | ✓ | ✓ (TV Rain) |
| Ходорковский | ✓ | ✓ (Ходорковский LIVE) |
| Рабкор | ✓ | ✓ |
| NewsMaker | ✓ | ✓ |
| NEXTA | — (сайт за анти-бот защитой) | ✓ (NEXTA Live) |

YouTube подключён через официальные RSS-фиды (`youtube.com/feeds/videos.xml?channel_id=...`) — API-ключи не нужны. Список источников: `backend/app/sources.py`.
...
- **api** — отдаёт ленту с фильтрами по источнику и типу (статья/видео). `GET /api/news/{id}` возвращает полный текст статьи: если его не было в RSS, текст извлекается из оригинальной страницы (trafilatura) при первом запросе и кэшируется в Redis.
- **bot** — шлёт подписчикам дайджест каждые N часов (только новое с прошлого дайджеста).
- **frontend** — SPA на Svelte, nginx проксирует `/api` на backend. Статьи читаются прямо на сайте (модальное окно с полным текстом и ссылкой на оригинал), видео — во встроенном YouTube-плеере.
...
# YouTube-канал (нужен channel_id, начинается с UC...):
yt("my-source-yt", "Моё СМИ (YouTube)", "UCxxxxxxxxxxxxxxxxxxxxxx"),
```

`channel_id` канала можно найти в исходном коде страницы канала (`externalId`) или через сервисы вроде commentpicker.com/youtube-channel-id.php.
